...
| `cms-model-maintenance.mjs` | Dry-run/apply reconciliation for safe many-to-many deletion rules and the canonical Dream Team Specialty catalogue. | `node --env-file=.env.cms scripts/cms-model-maintenance.mjs` (dry-run), then add `--apply` after review. |
